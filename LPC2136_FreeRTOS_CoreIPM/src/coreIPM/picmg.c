...
picmg_process_command( IPMI_PKT *pkt )
{
	 ...
	 */
	debug(3, "PICMG_CMD","CMD REQ 0x%x ",pkt->req->command);

	switch( pkt->req->command ) {
		case ATCA_CMD_GET_PICMG_PROPERTIES:	/* Get PICMG Properties */
			debug(3, "PICMG_CMD","GET PICMG PROPERTIES\n");
		...
		case ATCA_CMD_SET_PORT_STATE:		/* Set Port State */
			debug(3, "PICMG_CMD","SET PORT STATE");
		...
		case ATCA_CMD_SET_POWER_LEVEL:		/* Set Power Level */
			debug(3, "PICMG_CMD","SET POWER LEVEL");
		...
		case ATCA_CMD_RENEGOTIATE_POWER:	/* Renegotiate Power */
			debug(3, "PICMG_CMD","RENEGOTIATE POWER");
...
picmg_get_picmg_properties( IPMI_PKT *pkt )
{
	...
	resp->picmg_id = PICMG_ID;
	resp->picmg_extension_ver_minor = 0x1;
	resp->picmg_extension_ver_major = 0x4;
...
picmg_init( void )
{
	...
	// Init FRU0 (MMC FRU)
	fru[0].gen_status_led_prop = GS_LED_BLUE; //Blue led only!
	fru[0].led_capabilities[0].led_color_capabilities = LED_SUPPORT_BLUE;
	fru[0].led_capabilities[0].default_led_color=LED_COLOR_BLUE;
	fru[0].led_capabilities[0].default_led_color_override=LED_COLOR_BLUE;

	/* turn on the BLUE LED */
	gpio_led_on( GPIO_FRU_LED_BLUE );

	/* if the Insertion Criteria Met condition exists then we can go to M2 state */
	//TODO: This does nothing! Need to figure out why the MCH doesn't react to state change messages...
...
picmg_m2_state( unsigned fru_id )
{
	...
	info("PICMG_STATE","FRU %d in M2", fru_id);
...
picmg_m3_state( unsigned fru_id )
{
	...
	info("PICMG_STATE","FRU %d in M3", fru_id);
...
picmg_m4_state( unsigned fru_id )
{
	...
	info("PICMG_STATE","FRU %d in M4", fru_id);
...
picmg_m5_state( unsigned fru_id )
{
	...
	//gpio_led_blink( GPIO_ACTIVITY_LED, SHORT_BLINK_ON, SHORT_BLINK_OFF, 0 );
...
picmg_set_fru_activation_policy( IPMI_PKT *pkt )
{
	...
	debug( 3, "PICMG", "picmg_set_fru_activation_policy" );
...
picmg_set_fru_led_state( IPMI_PKT *pkt )
{
	...
	debug( 3, "PICMG", "set_fru_led_state:");
	debug( 3, "PICMG", "\tled:%x, func:%x", req->led_id,req->led_function);
	debug( 3, "PICMG", "\ton_duration:%d, color:%x\n",req->on_duration,req->color);
...
picmg_set_port_state( IPMI_PKT *pkt )
{
	...
	info("PICMG","Set Port %d to %s",req->link_info.channel_number,req->state?"ENABLE":"DISABLE");
