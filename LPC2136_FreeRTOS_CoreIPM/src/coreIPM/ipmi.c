...
void ipmi_process_libera_req( IPMI_PKT *pkt );
...
ipmi_process_pkt( IPMI_WS * ws ) 
{
	...
	switch( ws->incoming_protocol ) {
		...
		default:
			debug( 3, "PROC_PKT"," CC_NOT_SUPPORTED" );
	...
	if( completion_code == CC_NORMAL )
		debug( 3, "PROC_PKT","Processing request");
...
{
	debug(2,"IPMI_REQUEST", "netFN 0x%02X",pkt->hdr.netfn);

	switch( pkt->hdr.netfn ) {
		case NETFN_GROUP_EXTENSION_REQ:
			debug(2,"IPMI_REQUEST", "NETFN_GROUP_EXTENSION_REQ");
			picmg_process_command( pkt );
			break;
		case NETFN_APP_REQ:
			debug(2,"IPMI_REQUEST", "NETFN_APP_REQ");
			ipmi_process_app_req( pkt );
			break;
		case NETFN_EVENT_REQ:
			debug(2,"IPMI_REQUEST", "NETFN_EVENT_REQ");
			ipmi_process_event_req( pkt );
			break;
		case NETFN_FW_REQ:
			debug(2,"IPMI_REQUEST", "NETFN_FW_REQ");
			ipmi_process_fw_req( pkt );
			break;
		case NETFN_NVSTORE_REQ:
			debug(2,"IPMI_REQUEST", "NETFN_NVSTORE_REQ");
			ipmi_process_nvstore_req( pkt );
			break;
		case NETFN_LIBERA_REQ:
			debug(2,"IPMI_REQUEST", "NETFN_LIBERA_REQ");
			ipmi_process_libera_req( pkt );
			break;
		default:
			debug(2,"IPMI_REQUEST", "default");
...
{
	debug(2,"IPMI_REQUEST", "FW: INVALID_CMD\n");
...
{
	debug(2,"IPMI_REQUEST", "default: INVALID_CMD\n");
	pkt->resp->completion_code = CC_INVALID_CMD;
	pkt->hdr.resp_data_len = 0;
}
...
/*
 *   Mandatory Commands
 *   	Turn on Libera trigger buffers
 *   	Turn off Libera trigger buffers
 */
/*======================================================================*/

void
ipmi_process_libera_req( IPMI_PKT *pkt )
{
	HANDLE_LIBERA_REQ  *req  = (HANDLE_LIBERA_REQ * )(pkt->req);
	HANDLE_LIBERA_RESP *resp = (HANDLE_LIBERA_RESP *)(pkt->resp);

	switch( req->command )
	{
		case IPMI_LIBERA_CMD_SLOT_INFO:
			if ((req->param != IPMI_LIBERA_SLOT_7 &&
				 req->param != IPMI_LIBERA_SLOT_8 &&
				 req->param != 0
				) ||
				req->sign1 != IPMI_LIBERA_SIGN_REQ1 ||
				req->sign2 != IPMI_LIBERA_SIGN_REQ2
			) {
				debug(3,"IPMI_LIBERA", "IPMI_LIBERA_CMD_SLOT_INFO");
				debug(3,"IPMI_LIBERA", "INVALID DATA IN REQ");
				resp->completion_code = CC_INVALID_DATA_IN_REQ;
				pkt->hdr.resp_data_len = 0;
			} else if (g_module_state.payload_state != PAYLOAD_ACTIVE) {
				debug(3,"IPMI_LIBERA", "IPMI_LIBERA_CMD_SLOT_INFO");
				debug(3,"IPMI_LIBERA", "PAYLOAD NOT ACTIVE");
				resp->completion_code = CC_LIBERA_PAYLOAD_NOT_ACTIVE;
				pkt->hdr.resp_data_len = 0;
			} else {

				debug(3,"IPMI_LIBERA", "CMD SLOT INFO");
				debug(3,"IPMI_LIBERA", "SLOT %d", req->param);

				resp->completion_code = CC_NORMAL;
				resp->sign1 = IPMI_LIBERA_SIGN_RESP1;
				resp->sign2 = IPMI_LIBERA_SIGN_RESP2;

				/* !!! Action according to received slot information
				 * */
				if (req->param == IPMI_LIBERA_SLOT_7) {
					/*action, if required */
					g_module_state.ipmi_amc_host = AMC_FTRN_IN_LIBERA_SLOT_7;

				} else if (req->param == IPMI_LIBERA_SLOT_8) {
					/* action, if required */
					g_module_state.ipmi_amc_host = AMC_FTRN_IN_LIBERA_SLOT_8;

					// set LIBERA Trigger ports 6-7 to ON
					payload_port_set_state(FTRN_LIBERA_TRIGGER, "Libera Trigger",0,1);

				} else {
					// should not get here, but in any case
					// assume Libera Slot7 and do not enable Libera triggers
					g_module_state.ipmi_amc_host = AMC_FTRN_IN_LIBERA_SLOT_7;
				}

				pkt->hdr.resp_data_len = sizeof(HANDLE_LIBERA_RESP) - 1;
			}
		    break;

		case IPMI_LIBERA_CMD_RESET:
			if (req->param != IPMI_LIBERA_RESET_PAR ||
				req->sign1 != IPMI_LIBERA_SIGN_REQ1 ||
				req->sign2 != IPMI_LIBERA_SIGN_REQ2
			) {
				debug(3,"IPMI_LIBERA", "CMD PCIe RESET");
				debug(3,"IPMI_LIBERA", "INVALID DATA IN REQ");
				resp->completion_code = CC_INVALID_DATA_IN_REQ;
				pkt->hdr.resp_data_len = 0;
			} else {
				...
				debug(3,"IPMI_LIBERA", "CMD PCIE RESET");
				g_module_state.cmd = FTRN_CMD_PCIE_RESET;
		...
		default:
			debug(3, "IPMI_LIBERA", "Unknown command") ;
...
ipmi_process_app_req( IPMI_PKT *pkt )
{
	...
	debug(3,"IPMI_APP_REQ","Command 0x%02X",pkt->req->command);

	switch( pkt->req->command )
	{
		case IPMI_CMD_GET_DEVICE_ID: 
		    {
			debug(3,"IPMI_APP_CMD", "GET DEVICE ID");
			...
			gdi_resp->device_id = 0x1; //PICMG (unspecified)
...
//				DEV_SUP_IPMB_EVENT_GEN |
			...
			gdi_resp->aux_fw_rev[0] = 0x1;
			gdi_resp->aux_fw_rev[1] = 0x2;
			gdi_resp->aux_fw_rev[2] = 0x3;
			gdi_resp->aux_fw_rev[3] = 0x4;
		    }
		...
		case IPMI_CMD_COLD_RESET:
			...
			debug(3,"IPMI_APP_CMD","COLD RESET" );
		...
		case IPMI_CMD_WARM_RESET:
			...
			debug(3,"IPMI_APP_CMD","WARM RESET" );
		...
		case IPMI_CMD_GET_SELF_TEST_RESULTS: 
			...
			debug(3,"IPMI_APP_CMD","GET SELF TEST RESULTS" );
		...
		case IPMI_CMD_RESET_WATCHDOG_TIMER: 
			...
			debug(3,"IPMI_APP_CMD","RESET WATCHDOG TIMER\n" );
...
//				(SET_WATCHDOG_TIMER_CMD_RESP *)(pkt->resp);
			...
			debug(3,"IPMI_APP_CMD","SET WATCHDOG TIMER" );
		...
		case IPMI_CMD_GET_WATCHDOG_TIMER: 
			...
			debug(3,"IPMI_APP_CMD","GET WATCHDOG TIMER" );
		...
		default:
			...
			debug(3,"IPMI_APP_CMD","INVALID CMD: %2X", pkt->req->command);
...
ipmi_wd_expired(uchar * arg) 
{
	...
	if( wd_timer.pre_timeout_enabled ) {
		...
		debug(3,"IPMI_WD","wd expired: pre-timeout" );
...
ipmi_process_nvstore_req( IPMI_PKT *pkt )
{
	...
	switch( pkt->req->command ) {
		...
		default:
			debug(3,"IPMI_NV","CMD NOT SUPPORTED");
...
ipmi_read_fru_data( IPMI_PKT *pkt )
{
	...
	if( found ) {
		...
		if( resp->count_returned > 30 ) /* TODO check size */
			resp->count_returned = 30;
